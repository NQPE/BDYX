package com.custom.bdyx.widget.basepopup;
...
public abstract class BasePopupWindow implements ViewCreate {
    ...
    //------------------------------------------自定义位置的showPopup-----------------------------------------------
    //重写setPopupWindowCustomLocation来设置显示位置
    public void showPopupWindowCustomLocation(View view) {
        setPopupWindowCustomLocation(mPopupWindow,view);
        //再次获取一次元素 否则会出现未知原因无法获取一开始的初始化动画
        curAnima = getShowAnimation();
        curAnimator = getShowAnimator();
    ...
    /**
     * 自行通过mPopupWindow.showAtLocation或者showAsDropDown来设置显示位置
     * 然后通过showPopupWindowCustomLocation()函数调用
     *
     * @param mPopupWindow
     * @param view 目标view
     */
    protected abstract void setPopupWindowCustomLocation(PopupWindow mPopupWindow,View view);
